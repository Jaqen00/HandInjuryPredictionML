...
corr_data <- data
corr_data <-
  as.data.frame(lapply(corr_data, as.numeric), check.names = F)
colnames(corr_data) <-
  lapply(colnames(corr_data), function(x) {
    gsub('\\.', ' ', x)
  })
...
tiff(
  paste(out_col, 'Figures', 'correation plot.tiff', sep = '/'),
  width = 2000,
  height = 2000,
  res = 300
)
...
otu_train.cv <- result
otu_train.cv.df <-
  data.frame(sapply(otu_train.cv, '[[', 'error.cv'))
colnames(otu_train.cv.df) <-
  c("err1", "err2", "err3", "err4", "err5")
...
ggplot(otu_train.cv.df, aes(x = num)) +
  geom_line(aes_(y = as.name(names(otu_train.cv.df[1])), colour = 'Error at each step'), lwd =
              1.0) +
  geom_line(aes_(y = as.name(names(otu_train.cv.df[2])), colour = 'Error at each step'), lwd =
              1.0) +
  geom_line(aes_(y = as.name(names(otu_train.cv.df[3])), colour = 'Error at each step'), lwd =
              1.0) +
  geom_line(aes_(y = as.name(names(otu_train.cv.df[4])), colour = 'Error at each step'), lwd =
              1.0) +
  geom_line(aes_(y = as.name(names(otu_train.cv.df[5])), colour = 'Error at each step'), lwd =
              1.0) +
  geom_line(aes_(y = as.name(names(otu_train.cv.df[6])), colour = 'Mean error'), lwd =
              1.2) +
  scale_colour_manual(
    "",
    breaks = c('Error at each step', 'Mean error'),
    values = c('#02B1e6', '#e64602')
  ) +
  geom_vline(
    xintercept = 8,
    colour = 'gray',
  ...
  labs(x = "Number of variables",
       y = "Error rates of cross-validations",) +
  coord_trans(x = "reverse") +
  scale_x_continuous(breaks = seq(1, 17, 2)) +
  annotate(
    "text",
    x = 11,
    y = max(otu_train.cv.df$errmean) * 0.95,
    label = "Optimal = 8",
    size = 3,
    hjust = 0
  ) +
...
ggsave(
  paste(out_col, 'Figures', 'rfcv.png', sep = '/'),
  height = 3,
  width = 5,
  dpi = 600,
  bg = "transparent"
...
model_rf <-
  randomForest(formula(paste(out_col, '~.')), data = data[include_cols], ntree = 100)
imp <- importance(model_rf, type = 2, scale = F)
row.names(imp) <-
  lapply(row.names(imp), function(x) {
    gsub('\\.', ' ', x)
  })
featureImportance <-
...
rf_include_cols <-
  importance(model_rf, type = 2, scale = F) %>% {
    .[order(., decreasing = T), ]
  } %>%
  names %>% .[0:optimal] %>% append(out_col)
ggsave(
  paste(out_col, 'Figures', 'featureImp.tiff', sep = '/'),
  height = 3,
  width = 5,
  dpi = 300,
  bg = "transparent"
...
ggplot(df, aes(x = log(lambda), y = MSE)) +
  ...
  geom_vline(
    xintercept = c(log(fit_cv$lambda.1se), log(fit_cv$lambda.min)),
    linetype = "dashed",
    color = 'grey'
  ) +
  annotate(
    "text",
    x = log(1.1 * fit_cv$lambda.1se),
    y = max(fit_cv$cvup) * 0.94,
    label = expression(log('λ'['1se'])),
    size = 3,
    hjust = 0
  ) +
  annotate(
    "text",
    x = log(1.1 * fit_cv$lambda.min),
    y = max(fit_cv$cvup) * 0.94,
    label = expression(log("λ"['min'])),
    size = 3,
    hjust = 0
  ) +
  annotate(
    "text",
    x = log(1.1 * fit_cv$lambda.1se),
    y = max(fit_cv$cvup) * 0.92,
    label = paste0(df[df$lambda == fit_cv$lambda.1se, 'NZero'], ' coeff.'),
    size = 3,
    hjust = 0
  ) +
  annotate(
    ...
    x = log(1.1 * fit_cv$lambda.min),
    y = max(fit_cv$cvup) * 0.92,
    label = paste0(df[df$lambda == fit_cv$lambda.min, 'NZero'], ' coeff.'),
    size = 3,
    hjust = 0
  ) +
  theme(
...
ggsave(
  paste(out_col, 'Figures', 'lasso.tiff', sep = '/'),
  height = 3,
  width = 5,
  dpi = 300,
  device = 'tiff',
...
# Feature Selecting -- LASSO plot coef ------------------------------------

lasso.coef <- coef(fit_cv$glmnet.fit, s = fit_cv$lambda.1se, exact = F)
imp.df <-
  data.frame(features = lasso.coef@Dimnames[[1]][c(2:length(lasso.coef@Dimnames[[1]]))][lasso.coef@i],
             coef = lasso.coef@x[c(2:length(lasso.coef@x))])
...
ggsave(
  paste(out_col, 'Figures', 'lasso coef.tiff', sep = '/'),
  height = 3,
  width = 5,
  dpi = 300,
  device = 'tiff',
...
# Feature Selecting -- VARIABLE select ------------------------------------
include_cols <-
  imp.df[['features']] %>% append(rf_include_cols) %>% unique
# Model development -- Basic setting ------------------------------------
predictor <- levels(data[[out_col]])[2]
fml <-
  formula(paste(out_col, '~ ', paste0(
    setdiff(include_cols, out_col), collapse = ' + '
  )))
# Model development -- Split dataset ------------------------------------
...
write.csv(train_data,
          paste(out_col, 'data', 'train_data.csv', sep = '/'),
          row.names = F)
write.csv(test_data,
          paste(out_col, 'data', 'test_data.csv', sep = '/'),
          row.names = F)
...
custom_predict <- function(model) {
  train_probs <-
    predict(model,
            ...
            type = 'prob') %>% data.frame()
  test_probs <-
    predict(model, newdata = test_data, type = 'prob') %>% data.frame()
  train_raws <-
    predict(model,
            newdata = rbind(train_data, test_data),
            type = 'raw') %>% factor(levels = levels(train_data[[out_col]]))
  test_raws <-
    predict(model, newdata = test_data, type = 'raw') %>%  factor(levels = levels(test_data[[out_col]]))
  train_roc <-
    roc(rbind(train_data, test_data)[[out_col]], train_probs[[predictor]])
  test_roc <- roc(test_data[[out_col]], test_probs[[predictor]])
  train_auc <- round(ci.auc(train_roc), 4)
  test_auc <- round(ci.auc(test_roc), 4)
  train_matrix <-
    confusionMatrix(train_raws, rbind(train_data, test_data)[[out_col]], positive =
                      predictor)
  test_matrix <-
    confusionMatrix(test_raws, test_data[[out_col]], positive = predictor)
  list(
    train = list(
      probs = train_probs,
      raws = train_raws,
...
svm_grid = expand.grid(degree = seq(1, 5, 1),
                       scale = 1,
                       C = seq(0, 1, 0.2))
SVM_model <-
  train(
...
performance_matrix <-
  data.frame(Model = c('LR', 'RF', 'NB', 'SVM', 'XGboost'))
for (dataset in c('train', 'test')) {
  for (name in performance_matrix$Model) {
    eval(parse(
      text = paste0(
        "performance_matrix[performance_matrix$Model=='",
        name,
        "','",
        dataset,
        "_Accuracy'] <- ",
...
which.row <-
  sapply(as.list(c(2:ncol(performance_matrix))), function(col) {
    which.max(performance_matrix[, col])
  })
performance_matrix[, c(2:ncol(performance_matrix))] <-
  round(performance_matrix[, c(2:ncol(performance_matrix))], 4)
...
legend_labels <-
  sapply(c('LR', 'NB', 'RF', 'SVM', 'XGboost'), function(x) {
    paste0(x, " (AUC:", format(round(auc(
      roc(roc_data[roc_data$name == x, 'd'], roc_data[roc_data$name == x, 'm'])
    ), 4), nsmall = 4), ")")
  })
ggplot(roc_data, aes(m = m, d = d, color = name)) + geom_roc(n.cuts = 0,
                                                             size = 0.8,
                                                             max.num.points = 30) +  # ggtitle("ROC plots for 5 machine learning methods")+
  xlab('1 - Specificity') +
  ylab('Sensitivity') +
  labs(color = 'Models') +
  ggtitle('ROC Curve') +
...
ggsave(
  paste(out_col, 'Figures/roc_test.tiff', sep = '/'),
  height = 5,
  width = 5,
  dpi = 300,
  device = 'tiff',
